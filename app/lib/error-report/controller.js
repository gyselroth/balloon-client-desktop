...
const fsInfo = require('./fs-info.js');

module.exports = function(env, clientConfig) {
  function send() {
    logger.info('ErrorReport: sending error report triggered');
    ...
    return new Promise(function(resolve, reject) {
      ...
      archive.on('error', function(err) {
        logger.error('ErrorReport: sending error report failed', err);
        reject(err);
      });

      var req = request.put('https://support.gyselroth.net/balloon/' + reportName);
      ...
      req.on('response', function(response) {
        if(response.statusCode === 200) {
          logger.info('ErrorReport: got response ', {response});
          resolve(rerportName);
        } else {
          logger.error('ErrorReport: got response ', {response});
          reject();
        }
      });

      req.on('aborted', function(err) {
        logger.error('ErrorReport: request has been aborted by the server', err);
        reject(err);
      });
